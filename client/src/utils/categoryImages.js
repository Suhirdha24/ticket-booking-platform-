export const CATEGORY_IMAGE_POOLS = {
  ...
  Theatre: [
    ...
    'https://images.unsplash.com/photo-1516307365426-bea591f05011?w=1200&auto=format&fit=crop&q=80',
  ...
  Exhibition: [
    ...
    'https://images.unsplash.com/photo-1561214115-f2f134cc4912?w=1200&auto=format&fit=crop&q=80',
  ...
  'Kids & Family': [
    ...
    'https://images.unsplash.com/photo-1576761846153-c6f3224a4c11?w=1200&auto=format&fit=crop&q=80',
    'https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?w=1200&auto=format&fit=crop&q=80',
...
export function getEventImage(event) {
  if (!event) return DEFAULT_GENERIC_IMAGE;

  // If the event already has a specific custom image (not the old generic default)
  if (
    event.bannerUrl &&
    event.bannerUrl !== DEFAULT_GENERIC_IMAGE &&
    !event.bannerUrl.includes('photo-1540747913346-19e32dc3e97e')
  ) {
    return event.bannerUrl;
  }

  const pool = CATEGORY_IMAGE_POOLS[event.category] || CATEGORY_IMAGE_POOLS['Concert'];
  const key = event.title || event._id || event.name || 'default';

  // Compute deterministic hash index so the same event gets the same stunning image
  let hash = 0;
